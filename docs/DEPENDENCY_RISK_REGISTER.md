...
| DEP-07 | `web-vitals` | Declared GoogleChrome's Apache-2.0 `5.3.0` package directly for production field measurement; the same version was already deduplicated under `posthog-js`. | The standard non-attribution build is a 2.56 KiB gzip lazy chunk, the bundle guard rejects static loading, and the production audit remains at zero findings. | Approved for the release candidate; monitor upstream major releases. |
