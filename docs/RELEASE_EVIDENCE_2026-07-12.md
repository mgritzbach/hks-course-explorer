...
|---|---|---|
| Protected current master | Merge `0c986343d1d23f653710e39951379599ad451eab`; CI `29251206180` | The complete Quality gate passed, including dependency, lint, formatting, contract, architecture, unit, Python safety, build, header, bundle, and local-preview browser checks. |
| Isolated release candidate | Deploy workflow `29251444607` | Target configuration, catalogue parity, exact-commit build, Pages upload, and static smoke passed. Browser acceptance passed routes, all advertised HKS offerings, graph reset, and mobile/navigation. The Course Advisor check failed closed twice with HTTP 503 `AI_NOT_CONFIGURED` because the Preview environment did not provide `OPENROUTER_API_KEY`; production promotion and every post-promotion step were skipped. |
...
| G04 Security | 90% | 0 | Course Explorer database hardening, live security headers, and the current custom-domain response-level cache preflight are proven; Preview Course Advisor parity and remaining shared-project advisor ownership are not closed. |
