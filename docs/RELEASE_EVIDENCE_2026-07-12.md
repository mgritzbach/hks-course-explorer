...
## Current exact-master evidence — 2026-07-13
...
The canonical technical blocker is now Preview environment parity for the
zero-cost Course Advisor, not authority to mutate a Cloudflare zone cache TTL.
Preview must provide its own `OPENROUTER_API_KEY`, retain a hard-zero-cost
provider control, and expose the reviewed `CHAT_RATE_LIMITER` binding before
the exact-master deployment can be rerun. No percentage or binary goal status
changes on this evidence alone.
...
The historical deployment workflow was red on the older custom-domain cache
assertion. Current master now validates the reviewed response-level cache
contract without a zone mutation, and the current public custom-domain
preflight passes. The separately recorded production deployment and production
acceptance prove G05 behavior. The post-snapshot deployment proves the
telemetry code is present and the Pages rollback/re-promotion boundary is
exercised. Neither substitutes for Preview Course Advisor parity or a fully
green canonical promotion of current master.
...
| G04 Security | 92% | 0 | Course Explorer database hardening, including the live trigger search-path correction, live security headers, and the current custom-domain response-level cache preflight are proven; Preview Course Advisor parity and remaining shared-project advisor ownership are not closed. |
| G05 Navigation / usability | 100% | 1 | The custom production domain passed all five desktop/mobile visitor acceptance boundaries; historical master `167e204` independently passed the same five release-candidate browser boundaries after its non-visual telemetry change. |
| G06 Accessibility | 95% | 0 | Automated route-wide desktop/mobile WCAG, loading/error landmark, focus, and keyboard checks pass; documented manual exact-production keyboard/mobile acceptance is missing. |
...
| G09 Regression safety | 98% | 0 | Exact-commit CI, route/browser/a11y gates, functional custom-domain acceptance, an exact rollback to `f3ce866`, verified re-promotion of `19e854a`, and the current public cache preflight are complete. Exact current master still requires a green canonical promotion and the complete rollback/re-promotion smoke must then be repeated in both directions. |
...
| G11 Operations / deployment | 97% | 0 | Ownership, runbooks, functional production/browser smoke, sync, complete database recovery, functional Pages rollback mechanics, and the current public cache preflight are proven; exact-current-master canonical promotion, the subsequent complete rollback repeat, and successor/on-call acceptance remain incomplete. |

The evidence-weighted total is approximately 96.5%; five of eleven goals are
...
2. Configure the existing zero-cost Course Advisor in the Cloudflare Preview
   environment: provide its dedicated `OPENROUTER_API_KEY`, verify the reviewed
   `CHAT_RATE_LIMITER` binding, and retain the hard-zero-cost provider control.
   Then complete the canonical exact-master deployment workflow.
3. Repeat the exact Pages rollback and re-promotion after Preview parity;
