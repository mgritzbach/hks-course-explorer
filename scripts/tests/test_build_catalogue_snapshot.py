...
class CatalogueSnapshotTests(unittest.TestCase):
    def setUp(self):
        ...
        self.history = [
            {"id": "api-2025", "course_code_base": "API-101", "year": 2025, "has_eval": True},
            {"id": "pal-2024", "course_code_base": "PAL-117", "year": 2024, "has_eval": True},
            {"id": "mld-average", "course_code_base": "MLD-717-M", "year": 0, "is_average": True, "has_eval": True},
            {"id": "related", "course_code_base": "DPI-802", "year": 2024, "has_eval": True},
        ]
    ...
    def test_preserves_each_current_offering_id_even_when_codes_repeat(self):
        rows = self.snapshot.materialize_catalogue_snapshot(
            [
                {"id": "one", "course_code_base": "API-101"},
                {"id": "two", "course_code_base": "API-101"},
            ],
            self.history,
            self.aliases,
        )
        self.assertEqual([row["offering_id"] for row in rows], ["one", "two"])
        self.assertTrue(all(row["match_method"] == "exact_code" for row in rows))
    ...
    def test_uses_only_the_reviewed_alias_registry_for_renumbered_courses(self):
        row = self.snapshot.materialize_catalogue_snapshot(
            [{"id": "current", "course_code_base": "DPI-802-M"}], self.history, self.aliases
        )[0]
        self.assertEqual(row["match_status"], "verified")
        self.assertEqual(row["match_method"], "approved_alias")
    ...
    def test_nearby_suffixes_remain_unmatched_instead_of_inheriting_ratings(self):
        row = self.snapshot.materialize_catalogue_snapshot(
            [{"id": "suffix", "course_code_base": "DPI-802-M-D"}], self.history, self.aliases
        )[0]
