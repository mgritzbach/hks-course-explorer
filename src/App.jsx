...
async function fetchAllCoursesWithCache(onProgress) {
  ...
  try {
    sessionStorage.setItem(
      COURSES_CACHE_KEY,
      JSON.stringify({ ts: Date.now(), data: courses }),
    );
  } catch (_e) {
...
function buildMeta(courses) {
  const concentrations = [
    ...new Set(courses.map((c) => c.concentration).filter(Boolean)),
  ].sort();
  ...
  courses
    .filter(
      (c) =>
        c.year && c.metrics_raw?.Instructor_Rating != null && !c.is_average,
    )
    .forEach((c) => {
  ...
  const evalYears = [
    ...new Set(
      courses
        .filter((c) => c.has_eval && !c.is_average && c.year)
        .map((c) => c.year),
    ),
  ];
...
export default function App() {
    ...
    return (
      <div className="flex min-h-screen" style={{ background: "transparent" }}>
        ...
        >
          ...
          >
            <div
              className="skeleton-shimmer mb-3 h-4"
              style={{ width: "60%" }}
            />
            <div
              className="skeleton-shimmer mb-2 h-8"
              style={{ width: "45%" }}
            />
  ...
  const handleExportShortlist = () => {
    ...
    const starred = data.courses.filter(
      (c) => !c.is_average && favs.isFavorite(c.course_code_base),
    );
    const seen = new Set();
    ...
    const rows = deduped.map((c) => {
      ...
      return [
        ...
        c.metrics_pct?.Instructor_Rating != null
          ? Math.round(c.metrics_pct.Instructor_Rating)
          : "",
        c.metrics_pct?.Course_Rating != null
          ? Math.round(c.metrics_pct.Course_Rating)
          : "",
        c.metrics_pct?.Workload != null
          ? Math.round(c.metrics_pct.Workload)
          : "",
        c.n_respondents ?? "",
        c.last_bid_price ?? "",
      ...
      >
        <Routes>
          ...
          <Route
            path="/requirements"
            element={
              <ScheduleBuilder myDegreeMode courses={data?.courses || []} />
            }
          />
  ...
  const mobileBottomNav = (
    ...
    >
      ...
      <div
        ...
        style={{
          borderColor: "var(--line)",
          background: "var(--nav-shell-strong)",
        }}
      >
            ...
            {allNavItems
              ...
              .map((item) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `hub-nav-link${isActive ? " hub-active" : ""}`
                  }
                >
                  {item.label}
                </NavLink>
      ...
      >
        ...
        <div
          ...
          style={{
            borderColor: "var(--line)",
            background:
              "linear-gradient(180deg, rgba(165, 28, 48, 0.14), var(--panel-subtle))",
          }}
        >
          <p className="kicker">Harvard-inspired</p>
          <p
            className="serif-display mt-2 text-2xl font-semibold"
            style={{ color: "var(--text)" }}
          >
            {config.schoolCode}
          </p>
          ...
          .map((item) => (
            <NavLink
              ...
              style={({ isActive }) => ({
                ...
                boxShadow: isActive
                  ? "0 14px 30px rgba(165, 28, 48, 0.16)"
                  : "none",
              })}
            >
              {item.label}
