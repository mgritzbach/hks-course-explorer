  ...
  return (
    <div className="mb-2">
      ...
      <div
        ...
        style={{ background: 'var(--track-bg)', position: 'relative' }}
      >
    ...
    >
      <div className="shrink-0 border-b px-4 pb-3 pt-4" style={{ borderColor: 'var(--line)' }}>
        <div className="mb-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {filters > 0 && (
              <button onClick={resetFilters} className="text-[10px] text-muted hover:text-label">
                Reset
              </button>
            )}
            {mobile && onClose && (
        ...
        <div className="grid gap-2">
          ...
          <div data-tour="faculty-active-since">
            <p className="mb-1 text-[10px] uppercase tracking-wider text-muted">
              Last Active (exclude older)
            </p>
            <div className="select-wrap">
              <select
        ...
        <p className="mt-3 text-[10px] text-muted">
          {displayedProfs.length} of {allProfessors.length} instructors
        </p>
      </div>
        ...
        {displayedProfs.map((prof) => {
          ...
          return (
            <button
              ...
              style={{
                background: selected ? 'var(--panel-subtle)' : undefined,
                borderColor: 'var(--line)',
                borderLeft: selected ? '3px solid var(--accent)' : '3px solid transparent',
              }}
            >
              <p className="text-xs font-medium leading-tight text-label">
                {prof.professor_display}
              </p>
              <div className="mt-1 flex flex-wrap items-center gap-2">
                <span className="text-[10px] text-muted">
                  {prof.evalCourses} course{prof.evalCourses !== 1 ? 's' : ''}
                </span>
                {avg != null && (
                  <span
                      ...
                      background:
                        avg >= 75
                          ...
                          : avg >= 50
                            ...
                            : 'var(--danger-soft)',
                      color:
                        avg >= 75 ? 'var(--success)' : avg >= 50 ? 'var(--gold)' : 'var(--danger)',
                    }}
                  >
                    {fmtShort(avg, metricMode)} instr.
                  </span>
                )}
              </div>
            </button>
...
export default function Faculty({
  ...
  const professorRegistry = useMemo(() => {
    ...
    return [...registry.values()]
      .filter((prof) => prof.evalCourses > 0)
      .map((prof) => ({
        ...prof,
        concentrations: [...prof.concentrationSet].sort(),
        avgRawInstructor:
          prof.cntRawInstructor > 0
            ? Math.round((prof.sumRawInstructor / prof.cntRawInstructor) * 100) / 100
            : null,
        lastTaughtYear:
          Math.max(...prof.courses.map((c) => c.year || 0).filter((y) => y > 0), 0) || null,
      }))
      ...
      return {
        ...
        avgMetrics: Object.fromEntries(
          meta.metrics.map((metric) => [
            metric.key,
            cntMap[metric.key]
              ? Math.round((sumMap[metric.key] / cntMap[metric.key]) * 10) / 10
              : null,
          ]),
        ),
      }
  ...
  const displayedProfs = useMemo(() => {
    ...
    const list = allProfessors.filter((prof) => {
      if (query) {
        ...
        if (
          !(prof.professor_display || '').toLowerCase().includes(normalized) &&
          !(prof.professor || '').toLowerCase().includes(normalized)
        )
          return false
      }
      if (concentration !== 'All' && !prof.concentrations.includes(concentration)) return false
      if (minRatingValue !== null && (prof.avgMetrics?.Instructor_Rating ?? -1) < minRatingValue)
        return false
    ...
    switch (sortBy) {
      case 'rating_desc':
        list.sort(
          (a, b) =>
            (b.avgMetrics?.Instructor_Rating ?? -1) - (a.avgMetrics?.Instructor_Rating ?? -1),
        )
        break
      case 'course_rating_desc':
        list.sort(
          (a, b) => (b.avgMetrics?.Course_Rating ?? -1) - (a.avgMetrics?.Course_Rating ?? -1),
        )
        break
      case 'courses_desc':
  ...
  const selectedData = useMemo(
    () =>
      selectedProf ? allProfessors.find((prof) => prof.professor === selectedProf) || null : null,
    ...
    () =>
      selectedData
        ? selectedData.courses
            .filter((course) => course.has_eval && !course.is_average)
            .sort(
  ...
  const resetFilters = () => {
    ...
    setMinRating('any')
    setMinCourses('any')
    setTaughtSinceYear('any')
    setSortBy('name_asc')
    setQuery('')
  }
  const handleSelectProf = (prof) => {
    setSelectedProf(prof.professor)
    setSearchParams({ prof: prof.professor })
  }
  ...
  return (
    <div className="flex h-full min-h-0 overflow-hidden">
      ...
      />
      {selectedData && (
        <OnboardingTour steps={FACULTY_DETAIL_TOUR_STEPS} storageKey="hks-tour-faculty-detail" />
      )}
      <div
      ...
      >
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
          ...
          <button
            ...
            className="rounded-full border px-3 py-2 text-xs font-medium text-label md:hidden"
            style={{ borderColor: 'var(--line)', background: 'var(--panel-subtle)', minHeight: 44 }}
          >
            Browse Faculty
            {activeFilterCount({ concentration, minRating, minCourses, taughtSinceYear }) > 0
              ? ` (${activeFilterCount({ concentration, minRating, minCourses, taughtSinceYear })})`
              : ''}
          </button>
        </div>
        ...
        {!selectedData && (
          ...
          >
            ...
            <p className="mt-2 max-w-md text-sm">
              Choose any instructor from the list to see teaching history evaluation trends and
        ...
        {selectedData && (
          <>
            ...
            <div className="mb-6">
              <h2
                className="serif-display mb-1 text-3xl font-semibold md:text-[2.25rem]"
                style={{ color: 'var(--text)' }}
              >
                {selectedData.professor_display}
              </h2>
              ...
              {selectedData.concentrations.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-2">
                  {selectedData.concentrations.map((item) => (
                    <span
                      key={item}
              ...
              <div className="mt-3 flex flex-wrap gap-4 text-xs text-muted">
                <span>
                  {selectedData.evalCourses} course{selectedData.evalCourses !== 1 ? 's' : ''} with
                  evals
                </span>
            ...
            <div className="mb-6 grid gap-4 lg:grid-cols-2">
              <div data-tour="faculty-ratings" className="surface-card rounded-[22px] p-4">
                ...
                {meta.metrics
                  .filter((metric) => !metric.bid_metric)
                  .map((metric) => (
                    <MetricBar
                      key={metric.key}
                      label={metric.label}
                      value={selectedData.avgMetrics?.[metric.key]}
              ...
              <div data-tour="faculty-quick-stats" className="surface-card rounded-[22px] p-4">
                ...
                {selectedData.avgMetrics?.Instructor_Rating != null && (
                  ...
                  >
                    ...
                    <p className="text-xl font-bold" style={{ color: 'var(--accent-strong)' }}>
                      {fmtShort(selectedData.avgMetrics.Instructor_Rating, metricMode)}
                    </p>
                    <p className="text-[10px] text-muted">{modeSubLabel(metricMode)}</p>
                    {selectedData.avgRawInstructor != null && (
                      <p
                        className="mt-1 text-[10px]"
                        style={{ color: 'var(--text-muted)', opacity: 0.72 }}
                      >
                        avg {selectedData.avgRawInstructor.toFixed(2)}/5
                        {meta.overall_median_instructor != null && (
                          <span className="ml-1.5">
                            · all-courses med {meta.overall_median_instructor.toFixed(2)}
                          </span>
                        )}
                      </p>
                    )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { key: 'Course_Rating', label: 'Course Rating', color: 'var(--success)' },
                    { key: 'Workload', label: 'Workload', color: 'var(--text-soft)' },
                    { key: 'Rigor', label: 'Rigor', color: 'var(--text-soft)' },
                    {
                      key: 'Diverse Perspectives',
                      label: 'Diverse Persp.',
                      color: 'var(--text-soft)',
                    },
                    { key: 'Feedback', label: 'Feedback', color: 'var(--text-soft)' },
                  ].map(
                    ({ key, label, color }) =>
                      selectedData.avgMetrics?.[key] != null && (
                        <div
                          key={key}
                        ...
                        >
                          ...
                          <p className="text-sm font-bold" style={{ color }}>
                            {fmtShort(selectedData.avgMetrics[key], metricMode)}
                          </p>
                        </div>
                      ),
                  )}
            ...
            <div data-tour="faculty-courses-table" className="surface-card rounded-[22px]">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--line-strong)' }}>
                      {[
                        ...
                        'Term',
                        'Course',
                        `Instructor (${modeUnit(metricMode)})`,
                        `Course (${modeUnit(metricMode)})`,
                        `Workload (${modeUnit(metricMode)})`,
                        `Rigor (${modeUnit(metricMode)})`,
                        'Diverse Persp.',
                        'N',
                      ].map((h) => (
                        <th
                  ...
                  <tbody>
                    {profCourses.map((course) => {
                      ...
                      return (
                        <tr
                          key={`${course.year}-${course.term}-${course.course_code}`}
                        ...
                        >
                          <td className="px-3 py-2 text-label">{course.year}</td>
                          <td className="px-3 py-2 text-muted">{course.term}</td>
                          ...
                          <td className="px-3 py-2 font-medium" style={{ color: instrColor }}>
                            {fmtShort(instrVal, metricMode)}
                          </td>
                          <td className="px-3 py-2 text-label">
                            {fmtShort(src?.Course_Rating, metricMode)}
                          </td>
                          <td className="px-3 py-2 text-label">
                            {fmtShort(src?.Workload, metricMode)}
                          </td>
                          <td className="px-3 py-2 text-label">
                            {fmtShort(src?.Rigor, metricMode)}
                          </td>
                          <td className="px-3 py-2 text-label">
                            {fmtShort(src?.['Diverse Perspectives'], metricMode)}
                          </td>
                          <td className="px-3 py-2 text-muted">{course.n_respondents ?? '-'}</td>
                        </tr>
                      )
                    })}
                  </tbody>
                ...
                {meta.overall_median_instructor != null && (
                  <p
                    className="border-t px-4 py-2 text-[10px] text-muted"
                    style={{ borderColor: 'var(--line)' }}
                  >
                    Overall median instructor rating:{' '}
                    <span className="font-medium text-label">
                      {meta.overall_median_instructor.toFixed(2)}/5
                    </span>{' '}
